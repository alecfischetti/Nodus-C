...
static bool sendNotifications = false;
...
void readADC_routine(task* self, queue* ipc_queue)
{
    ...
    {
        filtered_adc[sensor] = read_adc(sensor+1);
    }
...
void readTemperature_routine(task* self, queue* ipc_queue)
{
    ...
    temperature = (3 * temperature + ADC_GetResult16(ADC_TEMPERATURE_CHANNEL)) / 4;
}
...
int main()
{
    ...
    readTemperature.is_active = true;
    ...
    switch(event)
    {
        ...
        case CYBLE_EVT_GATTS_WRITE_REQ: 							
            ...
            if(wrReqParam->handleValPair.attrHandle == 
                CYBLE_NODUSC_SERVICE_NODUSC_CHARACTERISTIC_CLIENT_CHARACTERISTIC_CONFIGURATION_DESC_HANDLE)
            {
...
void UpdateNotificationCCCD(void)
{
	...
	CCCDvalue[0] = sendNotifications;
	CCCDvalue[1] = 0x00;
	
	/* Update CCCD handle with notification status data*/
	NotificationCCCDhandle.attrHandle = CYBLE_NODUSC_SERVICE_NODUSC_CHARACTERISTIC_CLIENT_CHARACTERISTIC_CONFIGURATION_DESC_HANDLE;
	NotificationCCCDhandle.value.val = CCCDvalue;
	NotificationCCCDhandle.value.len = 2;
	
	/* Report data to BLE component for sending data when read by Central device */
	CyBle_GattsWriteAttributeValue(&NotificationCCCDhandle, 0, &cyBle_connHandle, CYBLE_GATT_DB_LOCALLY_INITIATED);
